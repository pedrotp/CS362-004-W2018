// Random test for the smithy card inside dominion.c
...
int main () {
  ...
  for (i = 0; i < NUMTESTS; i++) {
    ...
    G.hand[player][handPos] = smithy; // place smithy in the hand position generated

    // record values before playing smithy
    preDeck = G.deckCount[player];
    preHand = G.handCount[player];
    prePlayed = G.playedCardCount;

    // play smithy
    cardEffect(smithy, -1, -1 , -1, &G, handPos, NULL);

    // hand has two more cards
  ...
  // print test results
  printf("TEST 1: 'After adding three cards and discarding one, +2 cards in player's hand'\n");
  printf("FAILED -- %d (%.2f%%)\n", test1_f, (float)(test1_f * 100) / (float)NUMTESTS);
  printf("PASSED -- %d (%.2f%%)\n\n", test1_p, (float)(test1_p * 100) / (float)NUMTESTS);

  printf("TEST 2: 'Three cards removed from the player's deck'\n");
  ...
  printf("TEST 4: 'The last card played is smithy'\n");
